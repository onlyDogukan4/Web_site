(()=>{typeof window<"u"&&(window.moderraCart||(window.moderraCart=JSON.parse(localStorage.getItem("cart")||"[]").map(t=>({...t,price:isNaN(parseFloat(t.price))||parseFloat(t.price)<=0?15:parseFloat(t.price)}))));function c(){return typeof window<"u"?window.moderraCart:(global.moderraCart||(global.moderraCart=JSON.parse(localStorage.getItem("cart")||"[]").map(t=>({...t,price:isNaN(parseFloat(t.price))||parseFloat(t.price)<=0?15:parseFloat(t.price)}))),global.moderraCart)}function T(t){typeof window<"u"?window.moderraCart=t:global.moderraCart=t,h()}function h(){let t=c();typeof window<"u"&&localStorage.setItem("cart",JSON.stringify(t))}function u(){try{let t=localStorage.getItem("moderra_settings")||localStorage.getItem("settings");if(t)return JSON.parse(t)}catch{}return{minOrder:500,freeShipping:1e3}}async function L(){try{let t=await fetch("/api/settings?t="+Date.now());if(t.ok){let i=await t.json();localStorage.setItem("moderra_settings",JSON.stringify(i)),typeof updateCartDisplay=="function"&&updateCartDisplay()}}catch(t){console.warn("Ayarlar sunucudan al\u0131namad\u0131:",t)}}function v(){let t=0,i=0;c().forEach(r=>{let n=parseFloat(r.price)||0;if(r.isPackage&&Array.isArray(r.packageItems)){let s=r.packageItems.reduce((o,d)=>o+(parseFloat(d.price)||0)*(d.quantity||1),0);t+=s*(r.quantity||1),i+=s*(r.quantity||1)*((parseFloat(r.discount)||0)/100)}else t+=n*(r.quantity||1)});let e=Math.max(0,t-i);return{subTotal:t,discountTotal:i,total:e}}function k(t){let{freeShipping:i}=u();return t<i?150:0}function m(t){return`\u20BA${parseFloat(t).toLocaleString("tr-TR",{minimumFractionDigits:2,maximumFractionDigits:2})}`}function q(){let{subTotal:t,discountTotal:i,total:e}=v(),r=k(e),n=e+r;return{subTotal:t,discountTotal:i,productTotal:e,shipping:r,grandTotal:n,freeShippingLimit:u().freeShipping}}function E(){if(document.getElementById("moderra-cart-styles"))return;let t=document.createElement("style");t.id="moderra-cart-styles",t.textContent=`
        .cart-item-card {
    ...
    `,document.head.appendChild(t)}function g(){let t=document.getElementById("cart-items-list"),i=document.getElementById("cart-count");if(!t)return;let{minOrder:e,freeShipping:r}=u(),{subTotal:n,discountTotal:s,total:o}=v(),d=document.getElementById("cart-progress-sticky");if(d)if(o===0)d.innerHTML="";else if(o<e){let a=Math.min(o/e*100,100);d.innerHTML=`
            ...
            </div>`,i&&(i.style.display="none"),F(0,0,0);return}c().forEach(a=>{let l=document.createElement("div");if(l.dataset.cartItemId=a.id,a.isPackage){l.className="cart-item-card";let y=a.packageItems.reduce((f,x)=>f+parseFloat(x.price)*x.quantity,0)*(1-(a.discount||0)/100)*a.quantity;l.innerHTML=`
            ...
            `,l.style.background="linear-gradient(145deg, #fffdf5, #fef9e7)",l.style.outline="2px solid #d4af37",l.style.outlineOffset="0px",l.style.borderRadius="20px",l.style.marginBottom="14px",l.style.position="relative",l.style.overflow="visible",l.style.boxShadow="0 8px 32px rgba(212,175,55,0.25), inset 0 1px 0 rgba(255,255,255,0.8)";let f=a.logo&&a.logo.startsWith("data:application/pdf"),x=a.logo?`<div style="margin-top:10px;">
                ...
                </div>`}t.appendChild(l)}),F(n,s,o);let p=c().reduce((a,l)=>a+l.quantity,0);i&&(i.textContent=p,i.style.display=p>0?"flex":"none")}function F(t,i,e){let r=document.getElementById("cart-total-price-area");if(!r)return;let n=k(e),s=e+n,o=Math.round((s-s/1.2)*100)/100;r.innerHTML=`
...
`,s+=`\u{1F464} ${e.name}
\u{1F4DE} ${e.phone}
\u{1F3E0} ${e.address}`,window.open(`https://wa.me/905304640120?text=${encodeURIComponent(s)}`,"_blank")}function _(t,{compact:i=!1}={}){return`
        <div class="pay-breakdown" style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:14px;padding:${i?"10px 12px":"14px 16px"};font-size:${i?"12px":"13px"};">
            ...
            ${t.discountTotal>0?`<div style="display:flex;justify-content:space-between;margin-bottom:6px;color:#16a34a;font-weight:700;">
                <span>\u0130ndirim</span><span>\u2212${m(t.discountTotal)}</span>
            </div>`:""}
            <div style="display:flex;justify-content:space-between;margin-bottom:6px;color:#475569;">
                <span>\xDCr\xFCn tutar\u0131</span>
                <span>${m(t.productTotal)}</span>
            </div>
            <div style="display:flex;justify-content:space-between;margin-bottom:6px;color:${t.shipping>0?"#dc2626":"#16a34a"};font-weight:700;">
                ...
                <span>${t.shipping>0?m(t.shipping):"\xDCcretsiz"}</span>
            </div>
            ...
            <i class="fas fa-info-circle"></i> ${m(t)} i\xE7in g\xFCncel taksit se\xE7enekleri (PayTR). Kart\u0131n\u0131z\u0131 girdi\u011Finizde iframe i\xE7inde de g\xFCncel taksitler g\xF6r\xFCn\xFCr.
                ...
                <div class="paytr-head" style="padding:16px 22px;display:flex;justify-content:space-between;align-items:center;background:linear-gradient(135deg,#1e293b,#334155);flex-shrink:0;gap:12px;">
                    ...
                    <div style="display:flex;align-items:center;gap:16px;flex-shrink:0;">
                        <div class="paytr-head-total" style="text-align:right;">
                            ...
                            <p style="margin:0;font-size:20px;font-weight:900;color:#10b981;">${m(t.grandTotal)}</p>
                        </div>
                ...
                <div class="paytr-body" style="flex:1;display:flex;overflow:hidden;min-height:0;background:white;">
                    <div class="paytr-side" style="width:340px;flex-shrink:0;background:#f8fafc;border-right:1px solid #e2e8f0;display:flex;flex-direction:column;overflow-y:auto;">
                        <div style="padding:20px;">
                            <h4 style="margin:0 0 15px;font-size:14px;color:#1e293b;font-weight:800;">Sipari\u015F \xD6zeti</h4>
                            ${_(t)}
                        </div>
                        <div style="margin:0 20px 20px;background:white;border-radius:16px;border:1px solid #e2e8f0;overflow:hidden;box-shadow:0 4px 6px -1px rgba(0,0,0,0.05);">
                            <div style="padding:15px;background:linear-gradient(to right,#f8fafc,#f1f5f9);border-bottom:1px solid #e2e8f0;">
                                <p style="margin:0;font-size:12px;font-weight:800;color:#334155;"><i class="fas fa-credit-card" style="color:#6366f1;"></i> Taksit Se\xE7enekleri</p>
                            </div>
                    ...
                    <div class="paytr-frame" style="flex:1;display:flex;flex-direction:column;background:white;min-width:0;position:relative;">
                        <iframe src="https://www.paytr.com/odeme/guvenli/${d.token}" id="paytr-iframe" title="PayTR \xD6deme" style="width:100%;height:100%;flex:1;border:none;"></iframe>
                    </div>
                </div>
            ...
            `,document.head.appendChild(l)}p.querySelector("#paytr-modal-close").onclick=()=>p.remove();let a=document.getElementById("paytr-installments-panel");N(t.grandTotal,a,d.token,d.merchantId||"678000"),localStorage.setItem("last_order_id",d.orderId)}catch(o){console.error("PayTR hatas\u0131:",o),alert("\xD6deme sistemine ba\u011Flan\u0131lamad\u0131.")}finally{n&&(n.disabled=!1,n.innerHTML=s)}}function H(){document.body.classList.add("cart-open"),g()}function z(){document.body.classList.remove("cart-open")}function O(){document.getElementById("open-cart-modal")?.addEventListener("click",H),document.getElementById("cart-close-btn")?.addEventListener("click",z),document.getElementById("cart-overlay")?.addEventListener("click",z),document.addEventListener("keydown",t=>{t.key==="Escape"&&document.body.classList.contains("cart-open")&&z()})}function I(){document.hidden||L()}E();O();R();I();g();document.addEventListener("visibilitychange",()=>{document.hidden||I()});setInterval(I,6e4);Object.assign(window,{addToCart:j,addPackageToCart:P,removeFromCart:C,updateItemQuantity:B,updateCartDisplay:g,whatsappCheckout:M,payWithPayTR:D,flyToCart:w,openCart:H,closeCart:z});})();
