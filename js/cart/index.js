(()=>{typeof window<"u"&&(window.moderraCart||(window.moderraCart=JSON.parse(localStorage.getItem("cart")||"[]").map(t=>({...t,price:isNaN(parseFloat(t.price))||parseFloat(t.price)<=0?15:parseFloat(t.price)}))));function c(){return typeof window<"u"?window.moderraCart:(global.moderraCart||(global.moderraCart=JSON.parse(localStorage.getItem("cart")||"[]").map(t=>({...t,price:isNaN(parseFloat(t.price))||parseFloat(t.price)<=0?15:parseFloat(t.price)}))),global.moderraCart)}function T(t){typeof window<"u"?window.moderraCart=t:global.moderraCart=t,h()}function h(){let t=c();typeof window<"u"&&localStorage.setItem("cart",JSON.stringify(t))}function u(){try{let t=localStorage.getItem("moderra_settings")||localStorage.getItem("settings");if(t)return JSON.parse(t)}catch{}return{minOrder:500,freeShipping:1e3}}async function q(){try{let t=await fetch("/api/settings?t="+Date.now());if(t.ok){let n=await t.json();localStorage.setItem("moderra_settings",JSON.stringify(n)),typeof updateCartDisplay=="function"&&updateCartDisplay()}}catch(t){console.warn("Ayarlar sunucudan al\u0131namad\u0131:",t)}}function v(){let t=0,n=0;c().forEach(r=>{let i=parseFloat(r.price)||0;if(r.isPackage&&Array.isArray(r.packageItems)){let s=r.packageItems.reduce((o,d)=>o+(parseFloat(d.price)||0)*(d.quantity||1),0);t+=s*(r.quantity||1),n+=s*(r.quantity||1)*((parseFloat(r.discount)||0)/100)}else t+=i*(r.quantity||1)});let e=Math.max(0,t-n);return{subTotal:t,discountTotal:n,total:e}}function k(t){let{freeShipping:n}=u();return t<n?150:0}var L=.2;function g(t){return`\u20BA${parseFloat(t).toLocaleString("tr-TR",{minimumFractionDigits:2,maximumFractionDigits:2})}`}function E(){let{subTotal:t,discountTotal:n,total:e}=v(),r=k(e),i=e+r,s=i-i/(1+L);return{subTotal:t,discountTotal:n,productTotal:e,shipping:r,grandTotal:i,kdvAmount:Math.round(s*100)/100,kdvRatePercent:Math.round(L*100),freeShippingLimit:u().freeShipping}}function F(){if(document.getElementById("moderra-cart-styles"))return;let t=document.createElement("style");t.id="moderra-cart-styles",t.textContent=`
        .cart-item-card {
    ...
    `,document.head.appendChild(t)}function m(){let t=document.getElementById("cart-items-list"),n=document.getElementById("cart-count");if(!t)return;let{minOrder:e,freeShipping:r}=u(),{subTotal:i,discountTotal:s,total:o}=v(),d=document.getElementById("cart-progress-sticky");if(d)if(o===0)d.innerHTML="";else if(o<e){let a=Math.min(o/e*100,100);d.innerHTML=`
            ...
            <div style="text-align:center;padding:80px 20px;color:#cbd5e1;">
            ...
            </div>`,n&&(n.style.display="none"),R(0,0,0);return}c().forEach(a=>{let l=document.createElement("div");if(l.dataset.cartItemId=a.id,a.isPackage){l.className="cart-item-card";let y=a.packageItems.reduce((f,x)=>f+parseFloat(x.price)*x.quantity,0)*(1-(a.discount||0)/100)*a.quantity;l.innerHTML=`
                <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:14px;">
                    ...
                    <div style="text-align:right;flex-shrink:0;">
                        <div style="font-size:17px;font-weight:900;color:var(--primary);">\u20BA${y.toLocaleString("tr-TR",{minimumFractionDigits:2})}</div>
                        <button onclick="removeFromCart('${a.id}')" class="trash-btn">\u{1F5D1}</button>
                    </div>
                </div>`}else if(a.isConcept){let y=(parseFloat(a.price)||0)*(a.quantity||1);l.style.cssText=`
            ...
            `,l.style.background="linear-gradient(145deg, #fffdf5, #fef9e7)",l.style.outline="2px solid #d4af37",l.style.outlineOffset="0px",l.style.borderRadius="20px",l.style.marginBottom="14px",l.style.position="relative",l.style.overflow="visible",l.style.boxShadow="0 8px 32px rgba(212,175,55,0.25), inset 0 1px 0 rgba(255,255,255,0.8)";let f=a.logo&&a.logo.startsWith("data:application/pdf"),x=a.logo?`<div style="margin-top:10px;">
                ...
                <div style="padding:14px 16px 16px;">
                    <div style="display:flex;align-items:flex-start;gap:14px;">
                        ...
                        <div style="text-align:right;flex-shrink:0;">
                            ...
                            ">\u20BA${y.toLocaleString("tr-TR",{minimumFractionDigits:2})}</div>
                ...
                </div>`}else{l.className="cart-item-card";let y=parseFloat(a.price)*a.quantity;l.innerHTML=`
                ...
                </div>`}t.appendChild(l)}),R(i,s,o);let p=c().reduce((a,l)=>a+l.quantity,0);n&&(n.textContent=p,n.style.display=p>0?"flex":"none")}function R(t,n,e){let r=document.getElementById("cart-total-price-area");if(!r)return;let i=k(e),s=e+i,o=Math.round((s-s/1.2)*100)/100;r.innerHTML=`
        ...
        <div style="display:flex;justify-content:space-between;margin-bottom:6px;color:${i>0?"#dc2626":"#16a34a"};font-size:13px;font-weight:700;">
            <span>${i>0?"\u{1F69A} Kargo":"\u{1F389} Kargo (\xDCcretsiz)"}</span>
            <span>${i>0?"\u20BA"+i.toLocaleString("tr-TR"):"\xDCcretsiz"}</span>
        </div>
...
`}function P(){window.addEventListener("storage",t=>{t.key==="cart"&&(T(JSON.parse(t.newValue||"[]")),m())})}function w(t){if(!t)return;let n=document.getElementById("open-cart-modal");if(!n)return;let e=t.cloneNode(!0),r=t.getBoundingClientRect(),i=n.getBoundingClientRect();Object.assign(e.style,{position:"fixed",left:r.left+"px",top:r.top+"px",width:"80px",height:"80px",zIndex:"100000",pointerEvents:"none",transition:"all 0.75s cubic-bezier(0.19, 1, 0.22, 1)",borderRadius:"12px",objectFit:"contain"}),document.body.appendChild(e),requestAnimationFrame(()=>{e.style.transform=`translate(${i.left-r.left}px, ${i.top-r.top}px) scale(0.1)`,e.style.opacity="0.4"}),setTimeout(()=>{e.remove(),n.classList.add("shake"),setTimeout(()=>n.classList.remove("shake"),500)},750)}function S(t){let n=document.getElementById("vip-toast-overlay");n&&n.remove();let e=document.createElement("div");e.id="vip-toast-overlay",e.style.cssText=`
    ...
    `,document.body.appendChild(e),requestAnimationFrame(()=>{let r=document.getElementById("vip-toast-card");r&&(r.style.opacity="1",r.style.transform="scale(1) translateY(0)"),setTimeout(()=>{let i=document.getElementById("vip-toast-bar");i&&(i.style.width="0%")},100)}),setTimeout(()=>{let r=document.getElementById("vip-toast-card");r&&(r.style.opacity="0",r.style.transform="scale(0.9) translateY(-20px)"),setTimeout(()=>e.remove(),500)},3200)}function j(t,n,e,r=!1){let i=t;typeof t=="string"||typeof t=="number"?i={id:String(t),name:n,price:parseFloat(e),quantity:1,isConcept:r}:(i={...i},r&&(i.isConcept=!0),i.quantity||(i.quantity=1));let s=null,o=document.getElementById("product-detail-modal");o&&o.style.display==="block"&&(s=document.getElementById("detail-img")),s||(s=document.querySelector(`.add-to-cart[onclick*="'${i.id}'"]`)?.closest(".product-card")?.querySelector("img")),s&&w(s);let d=c().find(p=>p.id===i.id);d?d.quantity+=i.quantity||1:c().push(i),h(),setTimeout(()=>{m(),document.body.classList.add("cart-open")},800),i.isConcept&&typeof S=="function"&&S(i.name)}async function M(t){try{let[n,e]=await Promise.all([fetch("/api/packages"),fetch("/api/products")]),r=await n.json(),i=await e.json(),s=r.find(f=>String(f.id)===String(t));if(!s)return;let d=(s.items||"").split(",").map(f=>f.trim()).filter(Boolean).reduce((f,x)=>{let b=i.find($=>String($.id)===x);if(b){let $=(window.onCardQuantities||{})[`${t}-${b.id}`]||1;f.push({id:b.id,name:b.name_tr,price:b.price,image:b.image,quantity:$})}return f},[]),p=document.querySelector(`button[onclick*="addPackageToCart('${t}')"]`)?.closest(".package-card"),a=p?p.querySelector("img"):null;a&&w(a);let l=c().findIndex(f=>String(f.id)===String(s.id)),y=(s.name||"").toLowerCase().includes("s\xFCper");l>-1?y?(c()[l].packageItems=d,c()[l].quantity=1):c()[l].quantity++:c().push({id:String(s.id),name:s.name,image:s.image||"images/bardak.png",discount:s.discount||0,quantity:1,isPackage:!0,packageItems:d}),h(),setTimeout(()=>{m(),document.body.classList.add("cart-open")},800)}catch(n){console.error("addPackageToCart hatas\u0131:",n)}}function C(t){T(c().filter(n=>n.id!==t)),m()}function B(t,n){let e=c().find(r=>r.id===t);e&&(e.quantity=Math.max(0,e.quantity+n),e.quantity===0?C(t):(h(),m()))}function _(){let{minOrder:t}=u(),{total:n}=v();if(n<t){alert(`Minimum sipari\u015F tutar\u0131 \u20BA${t.toLocaleString("tr-TR")} TL'dir.`);return}let e=JSON.parse(localStorage.getItem("moderra_user_data")||"{}");if(!e.name||!e.phone||!e.address){alert("L\xFCtfen sipari\u015F i\xE7in profil bilgilerinizi doldurun.");let o=document.getElementById("profile-modal");o&&(o.style.display="block");return}let r=k(n),i=n+r,s=`*Moderra \u2014 Yeni Sipari\u015F*
...
`),s+=`
*\u{1F4B0} Toplam: \u20BA${i.toLocaleString("tr-TR",{minimumFractionDigits:2})}*

`,s+=`\u{1F464} ${e.name}
\u{1F4DE} ${e.phone}
\u{1F3E0} ${e.address}`,window.open(`https://wa.me/905304640120?text=${encodeURIComponent(s)}`,"_blank")}function D(t,{compact:n=!1}={}){return`
        <div class="pay-breakdown" style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:14px;padding:${n?"10px 12px":"14px 16px"};font-size:${n?"12px":"13px"};">
            ...
            <div style="display:flex;justify-content:space-between;padding-top:8px;margin-top:6px;border-top:2px solid #e2e8f0;font-weight:900;font-size:${n?"15px":"17px"};color:#4338ca;">
        ...
        </div>`}async function O(t,n,e,r){n.innerHTML='<p style="text-align:center;color:#64748b;font-size:12px;padding:12px;"><i class="fas fa-spinner fa-spin"></i> Taksit oranlar\u0131 y\xFCkleniyor\u2026</p>';try{let i=await fetch(`/api/paytr-installments?amount=${encodeURIComponent(t)}`),s=await i.json();if(!i.ok||!s.plans?.length)throw new Error(s.error||"Oran al\u0131namad\u0131");let o=`<p style="margin:0 0 10px;font-size:11px;color:#64748b;line-height:1.4;">
            <i class="fas fa-info-circle"></i> ${g(t)} i\xE7in g\xFCncel taksit se\xE7enekleri (PayTR). Kart\u0131n\u0131z\u0131 girdi\u011Finizde iframe i\xE7inde de g\xFCncel taksitler g\xF6r\xFCn\xFCr.
                ...
                </div>`;o+="</div></div>"}n.innerHTML=o}catch(i){console.warn("Taksit API:",i),n.innerHTML='<p style="font-size:11px;color:#94a3b8;">Taksit tablosu y\xFCklenemedi. Kart bilgilerinizi girdi\u011Finizde PayTR ekran\u0131nda taksit se\xE7enekleri g\xF6r\xFCnecektir.</p>'}if(e&&r&&!String(e).startsWith("mock_")){let i=document.createElement("script"),s=Math.round(t);i.src=`https://www.paytr.com/odeme/taksit-tablosu/v2?token=${encodeURIComponent(e)}&merchant_id=${encodeURIComponent(r)}&amount=${s}&taksit=12&tumu=1`,i.onload=()=>{let o=document.getElementById("paytr_taksit_tablosu_widget");o&&o.innerHTML.trim()&&(n.insertAdjacentHTML("beforeend",'<p style="margin:12px 0 6px;font-size:10px;font-weight:800;color:#4338ca;">PayTR resmi taksit tablosu</p><div id="paytr_taksit_tablosu_widget_wrap"></div>'),document.getElementById("paytr_taksit_tablosu_widget_wrap")?.appendChild(o))},document.body.appendChild(i)}}async function H(){let t=E(),{minOrder:n}=u();if(t.productTotal<n){alert(`Minimum sipari\u015F tutar\u0131 \u20BA${n.toLocaleString("tr-TR")} TL'dir.`);return}let e=JSON.parse(localStorage.getItem("moderra_user_data")||"{}");if(!e.name||!e.phone||!e.address){alert("L\xFCtfen \xF6deme i\xE7in profil bilgilerinizi doldurun.");let o=document.getElementById("profile-modal");o&&(o.style.display="block");return}if(!await new Promise(o=>{let d=document.createElement("div");d.style.cssText="position:fixed;inset:0;background:rgba(15,23,42,0.7);backdrop-filter:blur(8px);z-index:99998;display:flex;align-items:center;justify-content:center;padding:20px;",d.innerHTML=`
            ...
            <div class="paytr-shell" style="width:100%;max-width:1200px;background:#f8fafc;border-radius:28px;overflow:hidden;height:min(95vh,900px);display:flex;flex-direction:column;box-shadow:0 40px 80px rgba(0,0,0,0.5);">
                ...
                <div class="paytr-body" style="flex:1;display:flex;overflow:hidden;min-height:0;">
                    <div class="paytr-side" style="width:min(380px,42vw);flex-shrink:0;background:#f1f5f9;border-right:1px solid #e2e8f0;display:flex;flex-direction:column;overflow-y:auto;">
                        <div style="padding:14px 14px 0;">${D(t)}</div>
            ...
            `,document.head.appendChild(l)}p.querySelector("#paytr-modal-close").onclick=()=>p.remove();let a=document.getElementById("paytr-installments-panel");O(t.grandTotal,a,d.token,d.merchantId||"678000"),localStorage.setItem("last_order_id",d.orderId)}catch(o){console.error("PayTR hatas\u0131:",o),alert("\xD6deme sistemine ba\u011Flan\u0131lamad\u0131.")}finally{i&&(i.disabled=!1,i.innerHTML=s)}}function N(){document.body.classList.add("cart-open"),m()}function z(){document.body.classList.remove("cart-open")}function A(){document.getElementById("open-cart-modal")?.addEventListener("click",N),document.getElementById("cart-close-btn")?.addEventListener("click",z),document.getElementById("cart-overlay")?.addEventListener("click",z),document.addEventListener("keydown",t=>{t.key==="Escape"&&document.body.classList.contains("cart-open")&&z()})}function I(){document.hidden||q()}F();A();P();I();m();document.addEventListener("visibilitychange",()=>{document.hidden||I()});setInterval(I,6e4);Object.assign(window,{addToCart:j,addPackageToCart:M,removeFromCart:C,updateItemQuantity:B,updateCartDisplay:m,whatsappCheckout:_,payWithPayTR:H,flyToCart:w,openCart:N,closeCart:z});})();
