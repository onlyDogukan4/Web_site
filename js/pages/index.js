(()=>{typeof window<"u"&&(window.moderraCart||(window.moderraCart=JSON.parse(localStorage.getItem("cart")||"[]").map(t=>({...t,price:isNaN(parseFloat(t.price))||parseFloat(t.price)<=0?15:parseFloat(t.price)}))));function u(){return typeof window<"u"?window.moderraCart:(global.moderraCart||(global.moderraCart=JSON.parse(localStorage.getItem("cart")||"[]").map(t=>({...t,price:isNaN(parseFloat(t.price))||parseFloat(t.price)<=0?15:parseFloat(t.price)}))),global.moderraCart)}function C(t){typeof window<"u"?window.moderraCart=t:global.moderraCart=t,v()}function v(){let t=u();typeof window<"u"&&localStorage.setItem("cart",JSON.stringify(t))}function w(){try{let t=localStorage.getItem("moderra_settings")||localStorage.getItem("settings");if(t)return JSON.parse(t)}catch{}return{minOrder:500,freeShipping:1e3}}function I(){let t=0,e=0;u().forEach(n=>{let r=parseFloat(n.price)||0;if(n.isPackage&&Array.isArray(n.packageItems)){let o=n.packageItems.reduce((l,d)=>l+(parseFloat(d.price)||0)*(d.quantity||1),0);t+=o*(n.quantity||1),e+=o*(n.quantity||1)*((parseFloat(n.discount)||0)/100)}else t+=r*(n.quantity||1)});let i=Math.max(0,t-e);return{subTotal:t,discountTotal:e,total:i}}function T(t){let{freeShipping:e}=w();return t<e?150:0}function b(){let t=document.getElementById("cart-items-list"),e=document.getElementById("cart-count");if(!t)return;let{minOrder:i,freeShipping:n}=w(),{subTotal:r,discountTotal:o,total:l}=I(),d=document.getElementById("cart-progress-sticky");if(d)if(l===0)d.innerHTML="";else if(l<i){let a=Math.min(l/i*100,100);d.innerHTML=`
                ...
                </div>`}else if(l<n){let a=Math.min(l/n*100,100);d.innerHTML=`
            ...
            `,c.style.background="linear-gradient(145deg, #fffdf5, #fef9e7)",c.style.outline="2px solid #d4af37",c.style.outlineOffset="0px",c.style.borderRadius="20px",c.style.marginBottom="14px",c.style.position="relative",c.style.overflow="visible",c.style.boxShadow="0 8px 32px rgba(212,175,55,0.25), inset 0 1px 0 rgba(255,255,255,0.8)";let x=a.logo&&a.logo.startsWith("data:application/pdf"),f=a.logo?`<div style="margin-top:10px;">
                ...
                </div>`}t.appendChild(c)}),L(r,o,l);let p=u().reduce((a,c)=>a+c.quantity,0);e&&(e.textContent=p,e.style.display=p>0?"flex":"none")}function L(t,e,i){let n=document.getElementById("cart-total-price-area");if(!n)return;let r=T(i),o=i+r,l=Math.round((o-o/1.2)*100)/100;n.innerHTML=`
...
`}function $(t){if(!t)return;let e=document.getElementById("open-cart-modal");if(!e)return;let i=t.cloneNode(!0),n=t.getBoundingClientRect(),r=e.getBoundingClientRect();Object.assign(i.style,{position:"fixed",left:n.left+"px",top:n.top+"px",width:"80px",height:"80px",zIndex:"100000",pointerEvents:"none",transition:"all 0.75s cubic-bezier(0.19, 1, 0.22, 1)",borderRadius:"12px",objectFit:"contain"}),document.body.appendChild(i),requestAnimationFrame(()=>{i.style.transform=`translate(${r.left-n.left}px, ${r.top-n.top}px) scale(0.1)`,i.style.opacity="0.4"}),setTimeout(()=>{i.remove(),e.classList.add("shake"),setTimeout(()=>e.classList.remove("shake"),500)},750)}var s={products:[],packages:[],campaigns:[],lang:localStorage.getItem("lang")||"tr",category:"T\xFCm\xFC",page:1};window.onCardQuantities=window.onCardQuantities||{};function h(t,e,i){let n=s.products.find(g=>String(g.id)===String(t)),r=n?.name_tr||e||"\xDCr\xFCn",o=parseFloat(n?.price??i),l=n?.image||"images/bardak.png";if(isNaN(o))return;let d=null,p=document.getElementById("product-detail-modal");p&&p.style.display==="block"&&(d=document.getElementById("detail-img")),d||(d=document.querySelector(`.add-to-cart[onclick*="'${t}'"]`)?.closest(".product-card")?.querySelector("img")),d&&$(d);let a=u(),c=a.find(g=>String(g.id)===String(t));c?c.quantity++:a.push({id:String(t),name:r,image:l,price:o,quantity:1}),v(),b(),document.body.classList.add("cart-open")}function z(t){let e=s.packages.find(p=>String(p.id)===String(t));if(!e)return;let n=(e.items||"").split(",").map(p=>p.trim()).map(p=>{let a=s.products.find(g=>String(g.id)===p);if(!a)return null;let c=window.onCardQuantities[`${t}-${a.id}`]||1;return{id:a.id,name:a.name_tr,price:a.price,image:a.image,quantity:c}}).filter(Boolean),o=document.querySelector(`button[onclick*="addPackageToCart('${t}')"]`)?.closest(".package-card")?.querySelector("img");o&&$(o);let l=u(),d=l.findIndex(p=>String(p.id)===String(e.id));d>-1?e.name.toLowerCase().includes("s\xFCper")?(l[d].packageItems=n,l[d].quantity=1):l[d].quantity++:l.push({id:String(e.id),name:e.name,image:e.image||"images/bardak.png",discount:e.discount||0,quantity:1,isPackage:!0,packageItems:n}),v(),b(),document.body.classList.add("cart-open")}function B(){window.addToCartByMatch=t=>{let e=s.products.find(i=>i.name_tr.toLowerCase().includes(t.toLowerCase()));return e?(h(e.id,e.name_tr,e.price),{success:!0,name:e.name_tr,id:String(e.id)}):{success:!1}}}async function S(){try{let t=Date.now(),[e,i,n]=await Promise.all([fetch(`/api/products?t=${t}`),fetch(`/api/packages?t=${t}`),fetch(`/api/campaigns?t=${t}`)]),[r,o,l]=await Promise.all([e.json(),i.json(),n.json()]);Array.isArray(r)&&r.length&&(s.products=r),Array.isArray(o)&&(s.packages=o),Array.isArray(l)&&(s.campaigns=l)}catch(t){console.warn("Veri y\xFCkleme hatas\u0131:",t.message)}}function P({syncCartPrices:t,renderProducts:e,renderPackages:i}){let n=null,r=null;async function o(){if(!document.hidden)try{let p=await fetch("/api/settings?type=last-update&t="+Date.now());if(!p.ok)return;let a=await p.json();if(n===null){n=a.time;return}a.time!==n&&(n=a.time,await S(),t(),e(),i())}catch{}}function l(){r||(o(),r=setInterval(o,2e4))}function d(){r&&(clearInterval(r),r=null)}document.addEventListener("visibilitychange",()=>{document.hidden?d():l()}),l()}function F(t){q(),document.getElementById("lang-toggle")?.addEventListener("click",e=>{e.preventDefault(),s.lang=s.lang==="tr"?"en":"tr",localStorage.setItem("lang",s.lang),q(),t?.()})}function q(){let t=document.getElementById("lang-toggle");t&&(t.textContent=s.lang==="tr"?"EN":"TR"),document.querySelectorAll("[data-tr]").forEach(e=>{let i=e.getAttribute(`data-${s.lang}`);i&&(e.textContent=i)}),document.querySelectorAll("[data-tr-placeholder]").forEach(e=>{let i=s.lang==="tr"?e.getAttribute("data-tr-placeholder"):e.getAttribute("data-en-placeholder");i&&(e.placeholder=i)})}function y(){return s.lang==="en"}function A(){let t=location.pathname.split("/").pop()||"index.html";document.querySelectorAll(".nav-links a").forEach(e=>{e.classList.toggle("active",e.getAttribute("href")===t)})}function D(t){return"\u20BA"+Number(t).toLocaleString("tr-TR",{minimumFractionDigits:2,maximumFractionDigits:2})}function G(t){let e=`${t.name||""} ${t.id||""} ${t.description||""}`.toLowerCase();return/düğün|dugun|wedding|gelin|damat/.test(e)}function k(){let t=document.getElementById("packages-container");if(!t)return;let e=y(),i=s.packages.filter(n=>n.published).sort((n,r)=>(n.order||0)-(r.order||0));t.innerHTML="",i.forEach(n=>{let r=(n.items||"").split(",").map(a=>a.trim()).filter(Boolean),o=0;r.forEach(a=>{let c=s.products.find(g=>String(g.id)===a);c&&(o+=parseFloat(c.price))});let l=o*(1-(n.discount||0)/100),d=r.length===3?"package-items-grid items-3":"package-items-grid",p=document.createElement("div");p.className=G(n)?"package-card package-card--wedding":"package-card",p.innerHTML=`
                ...
                <div class="${d}">
                    ${r.map(a=>{let c=s.products.find(W=>String(W.id)===a);if(!c)return"";let g=`${n.id}-${c.id}`,x=window.onCardQuantities[g]||1,f=e&&c.name_en||c.name_tr;return`
                ...
                <div class="package-footer">
                    <div class="package-price-block">
                        <div class="package-price" id="pkg-price-${n.id}">${D(l)}</div>
            ...
            </button>`).join("")}}function Q(){let t=document.getElementById("search-bar"),e=document.getElementById("search-input"),i=document.getElementById("search-count");function n(){t.classList.remove("active"),e.value="",i&&(i.textContent=""),m()}document.getElementById("open-search")?.addEventListener("click",()=>{t.classList.add("active"),e.value="",i&&(i.textContent=""),setTimeout(()=>e.focus(),50)}),document.getElementById("close-search")?.addEventListener("click",n),document.addEventListener("keydown",r=>{r.key==="Escape"&&t.classList.contains("active")&&n()}),e?.addEventListener("input",()=>{let r=e.value.toLowerCase().trim(),o=document.querySelectorAll(".product-card"),l=0;o.forEach(d=>{let p=d.querySelector(".product-title")?.textContent.toLowerCase()||"",a=!r||p.includes(r);d.style.display=a?"":"none",a&&l++}),i&&(i.textContent=r?`${l} sonu\xE7`:"")})}function J(){let t=document.querySelector(".slides"),e=document.querySelectorAll(".slider-dot");if(!t||!e.length)return;let i=0,n=r=>{i=r,t.style.transform=`translateX(-${i*100}%)`,e.forEach((o,l)=>o.classList.toggle("active",l===i))};e.forEach((r,o)=>r.addEventListener("click",()=>n(o))),setInterval(()=>n((i+1)%e.length),5e3)}function K(){let t=!1,e=u().map(i=>{if(i.isConcept||i.isPackage)return i;let n=s.products.find(r=>String(r.id)===String(i.id));if(n){let r=parseFloat(n.price);if(!isNaN(r)&&r!==parseFloat(i.price))return t=!0,{...i,price:r}}return i});e=e.map(i=>{if(!i.isPackage||!Array.isArray(i.packageItems))return i;let n=!1,r=i.packageItems.map(o=>{let l=s.products.find(d=>String(d.id)===String(o.id));if(l){let d=parseFloat(l.price);if(!isNaN(d)&&d!==parseFloat(o.price))return n=!0,{...o,price:d}}return o});return n?(t=!0,{...i,packageItems:r}):i}),t&&(C(e),b())}function U(){let t=document.getElementById("theme-toggle");t&&(localStorage.getItem("theme")==="dark"&&(document.documentElement.setAttribute("data-theme","dark"),t.classList.replace("fa-moon","fa-sun")),t.addEventListener("click",()=>{let e=document.documentElement.getAttribute("data-theme")==="dark";document.documentElement.toggleAttribute("data-theme",!e),e?(document.documentElement.removeAttribute("data-theme"),localStorage.setItem("theme","light"),t.classList.replace("fa-sun","fa-moon")):(document.documentElement.setAttribute("data-theme","dark"),localStorage.setItem("theme","dark"),t.classList.replace("fa-moon","fa-sun"))}))}document.addEventListener("DOMContentLoaded",async()=>{U(),F(()=>{m(),k()}),J(),A(),j(),Q(),_(),B(),await S(),m(),k(),P({syncCartPrices:K,renderProducts:m,renderPackages:k})});Object.assign(window,{addToCart:h,addPackageToCart:z,openProductDetail:N,filterCategory:O,adjustPkgQty:M,renderProducts:m});Object.defineProperty(window,"_page",{get:()=>s.page,set:t=>{s.page=t}});})();
