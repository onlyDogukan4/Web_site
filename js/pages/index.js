(()=>{var v=JSON.parse(localStorage.getItem("cart")||"[]").map(t=>({...t,price:isNaN(parseFloat(t.price))||parseFloat(t.price)<=0?15:parseFloat(t.price)}));function u(){return v}function T(t){v=t,k()}function k(){localStorage.setItem("cart",JSON.stringify(v))}function $(){try{let t=localStorage.getItem("moderra_settings")||localStorage.getItem("settings");if(t)return JSON.parse(t)}catch{}return{minOrder:500,freeShipping:1e3}}function I(){let t=0,e=0;v.forEach(i=>{let r=parseFloat(i.price)||0;if(i.isPackage&&Array.isArray(i.packageItems)){let o=i.packageItems.reduce((d,l)=>d+(parseFloat(l.price)||0)*(l.quantity||1),0);t+=o*(i.quantity||1),e+=o*(i.quantity||1)*((parseFloat(i.discount)||0)/100)}else t+=r*(i.quantity||1)});let n=Math.max(0,t-e);return{subTotal:t,discountTotal:e,total:n}}function L(t){let{freeShipping:e}=$();return t<e?150:0}function b(){let t=document.getElementById("cart-items-list"),e=document.getElementById("cart-count");if(!t)return;let{minOrder:n,freeShipping:i}=$(),{subTotal:r,discountTotal:o,total:d}=I(),l=document.getElementById("cart-progress-sticky");if(l)if(d===0)l.innerHTML="";else if(d<n){let a=Math.min(d/n*100,100);l.innerHTML=`
                ...
                </div>`}else if(d<i){let a=Math.min(d/i*100,100);l.innerHTML=`
            ...
            </div>`,e&&(e.style.display="none"),z(0,0,0);return}u().forEach(a=>{let c=document.createElement("div");if(c.dataset.cartItemId=a.id,a.isPackage){c.className="cart-item-card";let g=a.packageItems.reduce((x,f)=>x+parseFloat(f.price)*f.quantity,0)*(1-(a.discount||0)/100)*a.quantity;c.innerHTML=`
            ...
            `,c.style.background="linear-gradient(145deg, #fffdf5, #fef9e7)",c.style.outline="2px solid #d4af37",c.style.outlineOffset="0px",c.style.borderRadius="20px",c.style.marginBottom="14px",c.style.position="relative",c.style.overflow="visible",c.style.boxShadow="0 8px 32px rgba(212,175,55,0.25), inset 0 1px 0 rgba(255,255,255,0.8)";let x=a.logo&&a.logo.startsWith("data:application/pdf"),f=a.logo?`<div style="margin-top:10px;">
                ...
                </div>`}t.appendChild(c)}),z(r,o,d);let p=u().reduce((a,c)=>a+c.quantity,0);e&&(e.textContent=p,e.style.display=p>0?"flex":"none")}function z(t,e,n){let i=document.getElementById("cart-total-price-area");if(!i)return;let r=L(n),o=n+r,d=Math.round((o-o/1.2)*100)/100;i.innerHTML=`
...
`}function S(t){if(!t)return;let e=document.getElementById("open-cart-modal");if(!e)return;let n=t.cloneNode(!0),i=t.getBoundingClientRect(),r=e.getBoundingClientRect();Object.assign(n.style,{position:"fixed",left:i.left+"px",top:i.top+"px",width:"80px",height:"80px",zIndex:"100000",pointerEvents:"none",transition:"all 0.75s cubic-bezier(0.19, 1, 0.22, 1)",borderRadius:"12px",objectFit:"contain"}),document.body.appendChild(n),requestAnimationFrame(()=>{n.style.transform=`translate(${r.left-i.left}px, ${r.top-i.top}px) scale(0.1)`,n.style.opacity="0.4"}),setTimeout(()=>{n.remove(),e.classList.add("shake"),setTimeout(()=>e.classList.remove("shake"),500)},750)}var s={products:[],packages:[],campaigns:[],lang:localStorage.getItem("lang")||"tr",category:"T\xFCm\xFC",page:1};window.onCardQuantities=window.onCardQuantities||{};function h(t,e,n){let i=s.products.find(g=>String(g.id)===String(t)),r=i?.name_tr||e||"\xDCr\xFCn",o=parseFloat(i?.price??n),d=i?.image||"images/bardak.png";if(isNaN(o))return;let p=document.querySelector(`.add-to-cart[onclick*="'${t}'"]`)?.closest(".product-card")?.querySelector("img");p&&S(p);let a=u(),c=a.find(g=>String(g.id)===String(t));c?c.quantity++:a.push({id:String(t),name:r,image:d,price:o,quantity:1}),k(),b()}function P(t){let e=s.packages.find(p=>String(p.id)===String(t));if(!e)return;let i=(e.items||"").split(",").map(p=>p.trim()).map(p=>{let a=s.products.find(g=>String(g.id)===p);if(!a)return null;let c=window.onCardQuantities[`${t}-${a.id}`]||1;return{id:a.id,name:a.name_tr,price:a.price,image:a.image,quantity:c}}).filter(Boolean),o=document.querySelector(`button[onclick*="addPackageToCart('${t}')"]`)?.closest(".package-card")?.querySelector("img");o&&S(o);let d=u(),l=d.findIndex(p=>String(p.id)===String(e.id));l>-1?e.name.toLowerCase().includes("s\xFCper")?(d[l].packageItems=i,d[l].quantity=1):d[l].quantity++:d.push({id:String(e.id),name:e.name,image:e.image||"images/bardak.png",discount:e.discount||0,quantity:1,isPackage:!0,packageItems:i}),k(),setTimeout(()=>{b(),document.body.classList.add("cart-open")},800)}function B(){window.addToCartByMatch=t=>{let e=s.products.find(n=>n.name_tr.toLowerCase().includes(t.toLowerCase()));return e?(h(e.id,e.name_tr,e.price),{success:!0,name:e.name_tr,id:String(e.id)}):{success:!1}}}async function E(){try{let t=Date.now(),[e,n,i]=await Promise.all([fetch(`/api/products?t=${t}`),fetch(`/api/packages?t=${t}`),fetch(`/api/campaigns?t=${t}`)]),[r,o,d]=await Promise.all([e.json(),n.json(),i.json()]);Array.isArray(r)&&r.length&&(s.products=r),Array.isArray(o)&&(s.packages=o),Array.isArray(d)&&(s.campaigns=d)}catch(t){console.warn("Veri y\xFCkleme hatas\u0131:",t.message)}}function q({syncCartPrices:t,renderProducts:e,renderPackages:n}){let i=null,r=null;async function o(){if(!document.hidden)try{let p=await fetch("/api/settings?type=last-update&t="+Date.now());if(!p.ok)return;let a=await p.json();if(i===null){i=a.time;return}a.time!==i&&(i=a.time,await E(),t(),e(),n())}catch{}}function d(){r||(o(),r=setInterval(o,2e4))}function l(){r&&(clearInterval(r),r=null)}document.addEventListener("visibilitychange",()=>{document.hidden?l():d()}),d()}function A(t){F(),document.getElementById("lang-toggle")?.addEventListener("click",e=>{e.preventDefault(),s.lang=s.lang==="tr"?"en":"tr",localStorage.setItem("lang",s.lang),F(),t?.()})}function F(){let t=document.getElementById("lang-toggle");t&&(t.textContent=s.lang==="tr"?"EN":"TR"),document.querySelectorAll("[data-tr]").forEach(e=>{let n=e.getAttribute(`data-${s.lang}`);n&&(e.textContent=n)}),document.querySelectorAll("[data-tr-placeholder]").forEach(e=>{let n=s.lang==="tr"?e.getAttribute("data-tr-placeholder"):e.getAttribute("data-en-placeholder");n&&(e.placeholder=n)})}function y(){return s.lang==="en"}function D(){let t=location.pathname.split("/").pop()||"index.html";document.querySelectorAll(".nav-links a").forEach(e=>{e.classList.toggle("active",e.getAttribute("href")===t)})}function M(t){return"\u20BA"+Number(t).toLocaleString("tr-TR",{minimumFractionDigits:2,maximumFractionDigits:2})}function G(t){let e=`${t.name||""} ${t.id||""} ${t.description||""}`.toLowerCase();return/düğün|dugun|wedding|gelin|damat/.test(e)}function w(){let t=document.getElementById("packages-container");if(!t)return;let e=y(),n=s.packages.filter(i=>i.published).sort((i,r)=>(i.order||0)-(r.order||0));t.innerHTML="",n.forEach(i=>{let r=(i.items||"").split(",").map(a=>a.trim()).filter(Boolean),o=0;r.forEach(a=>{let c=s.products.find(g=>String(g.id)===a);c&&(o+=parseFloat(c.price))});let d=o*(1-(i.discount||0)/100),l=r.length===3?"package-items-grid items-3":"package-items-grid",p=document.createElement("div");p.className=G(i)?"package-card package-card--wedding":"package-card",p.innerHTML=`
                ...
                <div class="${l}">
                    ${r.map(a=>{let c=s.products.find(W=>String(W.id)===a);if(!c)return"";let g=`${i.id}-${c.id}`,x=window.onCardQuantities[g]||1,f=e&&c.name_en||c.name_tr;return`
                ...
                <div class="package-footer">
                    <div class="package-price-block">
                        <div class="package-price" id="pkg-price-${i.id}">${M(d)}</div>
                ...
                </div>`,t.appendChild(p)})}function _(t,e,n){let i=`${t}-${e}`;window.onCardQuantities[i]=Math.max(1,(window.onCardQuantities[i]||1)+n);let r=document.getElementById(`qty-${i}`);r&&(r.textContent=window.onCardQuantities[i]);let o=s.packages.find(p=>String(p.id)===String(t));if(!o)return;let d=0;(o.items||"").split(",").map(p=>p.trim()).forEach(p=>{let a=s.products.find(c=>String(c.id)===p);a&&(d+=parseFloat(a.price)*(window.onCardQuantities[`${t}-${a.id}`]||1))});let l=document.getElementById(`pkg-price-${t}`);l&&(l.textContent=M(d*(1-(o.discount||0)/100)))}function Y(t){let e="";for(let i=0;i<Math.floor(t);i++)e+='<i class="fas fa-star"></i>';t%1>=.5&&(e+='<i class="fas fa-star-half-alt"></i>');let n=5-Math.ceil(t);for(let i=0;i<n;i++)e+='<i class="far fa-star"></i>';return e}function j(){document.querySelector(".detail-close")?.addEventListener("click",()=>{document.getElementById("product-detail-modal").style.display="none"})}function R(t){let e=s.products.find(i=>String(i.id)===String(t));if(!e)return;let n=y();document.getElementById("detail-img").src=e.image||"images/bardak.png",document.getElementById("detail-title").textContent=n&&e.name_en||e.name_tr,document.getElementById("detail-price").textContent="\u20BA"+parseFloat(e.price).toLocaleString("tr-TR",{minimumFractionDigits:2}),document.getElementById("detail-rating").innerHTML=Y(parseFloat(e.rating)||5),document.getElementById("detail-desc").textContent=n?e.description_en||e.description_tr||"":e.description_tr||"",document.getElementById("detail-add-btn").onclick=()=>{h(e.id,e.name_tr,e.price),document.getElementById("product-detail-modal").style.display="none"},document.getElementById("product-detail-modal").style.display="block"}function N(){let t=document.getElementById("profile-modal");document.getElementById("open-profile-modal")?.addEventListener("click",()=>{t.style.display="block"}),document.querySelector(".profile-close")?.addEventListener("click",()=>{t.style.display="none"}),document.getElementById("profile-form")?.addEventListener("submit",n=>{n.preventDefault(),localStorage.setItem("moderra_user_data",JSON.stringify({name:document.getElementById("user-name").value,phone:document.getElementById("user-phone").value,address:document.getElementById("user-address").value})),t.style.display="none"});let e=JSON.parse(localStorage.getItem("moderra_user_data")||"{}");e.name&&(document.getElementById("user-name").value=e.name),e.phone&&(document.getElementById("user-phone").value=e.phone),e.address&&(document.getElementById("user-address").value=e.address),window.addEventListener("click",n=>{n.target===t&&(t.style.display="none");let i=document.getElementById("product-detail-modal");n.target===i&&(i.style.display="none")})}var Q={active:"Standart",hot:"F\u0131rsat",discount:"\u0130ndirimli",oos:"Stok D\u0131\u015F\u0131"};function m(){let t=document.getElementById("products-container");if(!t)return;let e=y(),n=s.category==="T\xFCm\xFC"?s.products:s.products.filter(o=>(Q[o.status]||"Standart")===s.category),i=Math.max(1,Math.ceil(n.length/6));s.page>i&&(s.page=1);let r=n.slice((s.page-1)*6,s.page*6);if(X(),t.innerHTML="",!r.length){t.innerHTML=`<div style="grid-column:1/-1;text-align:center;padding:60px;color:var(--text-muted);">
                <i class="fas fa-box-open fa-3x" style="opacity:.3;margin-bottom:15px;display:block;"></i>
                Bu kategoride \xFCr\xFCn bulunamad\u0131.
            </div>`,H(i);return}r.forEach(o=>{let d=e&&o.name_en||o.name_tr,l=o.status==="oos",p=s.campaigns.find(g=>String(g.id)===String(o.campaign_id)),a="";o.status==="hot"&&!p&&(a='<div class="hot-badge"><i class="fas fa-fire"></i> FIRSAT</div>'),l&&(a='<div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%) rotate(-15deg);background:rgba(0,0,0,0.8);color:white;padding:10px 20px;font-weight:900;z-index:30;border-radius:10px;border:2px solid #ef4444;font-size:14px;">T\xDCKEND\u0130</div>');let c=document.createElement("div");c.className="product-card"+(l?" oos-card":""),c.style.position="relative",c.innerHTML=`
                ...
                </div>`,t.appendChild(c)}),H(i)}function X(){let t=document.getElementById("product-category-nav");if(!t)return;let e=["T\xFCm\xFC",...new Set(s.products.map(n=>Q[n.status]||"Standart"))];t.innerHTML=e.map(n=>`
            ...
            </button>`).join("")}function O(t){s.category=t,s.page=1,m()}function H(t){let e=document.getElementById("product-pagination");if(e){if(t<=1){e.innerHTML="";return}e.innerHTML=Array.from({length:t},(n,i)=>i+1).map(n=>`
            ...
            </button>`).join("")}}function K(){let t=document.getElementById("search-bar"),e=document.getElementById("search-input"),n=document.getElementById("search-count");function i(){t.classList.remove("active"),e.value="",n&&(n.textContent=""),m()}document.getElementById("open-search")?.addEventListener("click",()=>{t.classList.add("active"),e.value="",n&&(n.textContent=""),setTimeout(()=>e.focus(),50)}),document.getElementById("close-search")?.addEventListener("click",i),document.addEventListener("keydown",r=>{r.key==="Escape"&&t.classList.contains("active")&&i()}),e?.addEventListener("input",()=>{let r=e.value.toLowerCase().trim(),o=document.querySelectorAll(".product-card"),d=0;o.forEach(l=>{let p=l.querySelector(".product-title")?.textContent.toLowerCase()||"",a=!r||p.includes(r);l.style.display=a?"":"none",a&&d++}),n&&(n.textContent=r?`${d} sonu\xE7`:"")})}function J(){let t=document.querySelector(".slides"),e=document.querySelectorAll(".slider-dot");if(!t||!e.length)return;let n=0,i=r=>{n=r,t.style.transform=`translateX(-${n*100}%)`,e.forEach((o,d)=>o.classList.toggle("active",d===n))};e.forEach((r,o)=>r.addEventListener("click",()=>i(o))),setInterval(()=>i((n+1)%e.length),5e3)}function V(){let t=!1,e=u().map(n=>{if(n.isConcept||n.isPackage)return n;let i=s.products.find(r=>String(r.id)===String(n.id));if(i){let r=parseFloat(i.price);if(!isNaN(r)&&r!==parseFloat(n.price))return t=!0,{...n,price:r}}return n});e=e.map(n=>{if(!n.isPackage||!Array.isArray(n.packageItems))return n;let i=!1,r=n.packageItems.map(o=>{let d=s.products.find(l=>String(l.id)===String(o.id));if(d){let l=parseFloat(d.price);if(!isNaN(l)&&l!==parseFloat(o.price))return i=!0,{...o,price:l}}return o});return i?(t=!0,{...n,packageItems:r}):n}),t&&(T(e),b())}function U(){let t=document.getElementById("theme-toggle");t&&(localStorage.getItem("theme")==="dark"&&(document.documentElement.setAttribute("data-theme","dark"),t.classList.replace("fa-moon","fa-sun")),t.addEventListener("click",()=>{let e=document.documentElement.getAttribute("data-theme")==="dark";document.documentElement.toggleAttribute("data-theme",!e),e?(document.documentElement.removeAttribute("data-theme"),localStorage.setItem("theme","light"),t.classList.replace("fa-sun","fa-moon")):(document.documentElement.setAttribute("data-theme","dark"),localStorage.setItem("theme","dark"),t.classList.replace("fa-moon","fa-sun"))}))}document.addEventListener("DOMContentLoaded",async()=>{U(),A(()=>{m(),w()}),J(),D(),N(),K(),j(),B(),await E(),m(),w(),q({syncCartPrices:V,renderProducts:m,renderPackages:w})});Object.assign(window,{addToCart:h,addPackageToCart:P,openProductDetail:R,filterCategory:O,adjustPkgQty:_,renderProducts:m});Object.defineProperty(window,"_page",{get:()=>s.page,set:t=>{s.page=t}});})();
